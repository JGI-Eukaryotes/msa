...
var _zomeBioIo2 = require("zome-bio-io");
...
var Exporter = { openInJalview: function openInJalview(url, colorscheme) {
  ...
  publishWeb: function publishWeb(that, cb) {
    var text = _zomeBioIo2.fasta.write(that.seqs.toJSON());
    text = encodeURIComponent(text);
    var url = "http://sprunge.biojs.net";
    return (0, _zomeBioIo2.xhr)({
      method: "POST",
      body: "sprunge=" + text,
      uri: url,
  ...
  saveOriginalAsFile: function saveOriginalAsFile(that, name) {
    // limit at about 256k
    var text = _zomeBioIo.fasta.write(that.seqs.toJSON());
  ...
  saveAsFile: function saveAsFile(that, name) {
    // limit at about 256k
    var inView = that.g.seqs.models.filter(function (model) {
      return !model.changed.hidden;
    }).map(function (model) {
      return model.attributes;
    });
    var text = _zomeBioIo2.fasta.write(inView);
    var blob = new Blob([text], { type: 'text/plain' });
    return saveAs(blob, name);
  },
  ...
  saveSelection: function saveSelection(that, name) {
    ...
    var text = _zomeBioIo2.fasta.write(selection);
    var blob = new Blob([text], { type: 'text/plain' });
    return saveAs(blob, name);
  },
  ...
  saveAnnots: function saveAnnots(that, name) {
    ...
    var text = _zomeBioIo2.gff.exportLines(features);
    var blob = new Blob([text], { type: 'text/plain' });
    return saveAs(blob, name);
  },
